"""Cross-process MPI_Allreduce with a PRODUCT reduction.
...
stablehlo_text = f"""
module @verify_allreduce_product attributes {{mhlo.num_partitions = 1 : i32, mhlo.num_replicas = {size} : i32}} {{
  func.func public @main(%arg0: tensor<f32>) -> tensor<f32> {{
    %0 = "stablehlo.all_reduce"(%arg0) ({{
    ^bb0(%a: tensor<f32>, %b: tensor<f32>):
      %s = stablehlo.multiply %a, %b : tensor<f32>
...
# rank+2 (not rank+1) so no rank contributes a 0 factor, which would make the
# product trivially 0 regardless of whether PRODUCT reduction is threaded
# through correctly at all -- a weak test wouldn't distinguish "broken" from
# "coincidentally 0".
local_data = jax.device_put(np.array(float(rank + 2), dtype=np.float32), jax.devices()[rank])
result = executable.execute([local_data])
got = np.asarray(result[0])
expected = float(np.prod([r + 2 for r in range(size)]))
